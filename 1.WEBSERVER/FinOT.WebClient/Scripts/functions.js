...
//    }
    ...
    $('.application-form').each(function() {
      ...
      $form.submit(function() {
        ...
        if(isValid) {
          $('.field-input.required', $form).each(function() {
            if($(this).val().replace(/^\s*|\s*$/g,"")=="") {
              ...
              err_msg += '<li><a href="javascript:void();" data-id="'+$(this).attr('id')+'">'+$(this).data('error')+'</a></li>';
              err_cnt++;
            }
          });
        }
        ...
        if(!(isValid)) {
          err_msg = '<div class="error-header">'+err_cnt+' error'+((err_cnt>1) ? 's':'')+' occured while processing your submission</div><div class="error-content"><p>Please select the specific error item below to navigate to the field and correct your input.</p>'+err_msg+'</div></div>';
          
          $errorMessages.html(err_msg).show();
          scrollToElement($errorMessages, 800, -100);
          
          $('.error-message a').each(function() {
            var $item = $(this);
